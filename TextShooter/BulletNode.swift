...
class BulletNode: SKNode {
    ...
    override init() {
        ...
        body.contactTestBitMask = EnmeyCategory
        body.collisionBitMask = EnmeyCategory
    ...
    class func bullet(from start:CGPoint,toward destination:CGPoint) -> BulletNode{
        ...
        let scaleMovement = vectorMultiply(movement,1/magnitude)
        let thrustMagnititude = CGFloat(100.0)
        bullet.thrust = vectorMultiply(scaleMovement, thrustMagnititude)
